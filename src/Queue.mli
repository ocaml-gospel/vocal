...
(*@ add x q
      requires length q.view < max_int
      modifies q.view
...
(*@ r = take_opt q
      modifies q
      ensures  old q.view == empty -> r = None && q.view == old q.view
      ensures  not (old q.view == empty) ->
               r = Some (old q.view[0])
               && old q.view == cons (old q.view[0]) q.view *)
...
(*@ r = peek_opt q
      ensures  q.view == empty -> r = None
      ensures  not (q.view == empty) -> r = Some (q.view[0]) *)
...
(*@ transfer q1 q2
      requires length q1.view + length q2.view <= max_int
